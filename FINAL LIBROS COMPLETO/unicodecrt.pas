  ...
  Case C^ of
    ...
    #07 : begin
      ...
      WriteConsoleOutputCharacter(GetStdhandle(STD_OUTPUT_HANDLE),
        c, CodePointSize(c), writePos, numWritten);
...
begin
  ...
  for ch in s do
    begin
      //ws1 := ch+': '+IntToStr(Length(ch))+': '+IntToStr(getConsoleOutputCP)+': '+IntToStr(StringCodePage(ch));
      wc := ch;
      //MessageBoxW(0,@ws1[1],@ws2[1],0);
      Inc(WritePos.X);
      h := GetStdhandle(STD_OUTPUT_HANDLE);
      //does not work: WriteConsoleOutputCharacter(GetStdhandle(STD_OUTPUT_HANDLE), @ch[1], CodePointSize(@ch[1]), writePos, numWritten);
      WriteConsoleOutputCharacterW(GetStdhandle(STD_OUTPUT_HANDLE), @wc[1], CodePointSize(@wc[1]), writePos, numWritten);
      //works: writefile(GetStdhandle(STD_OUTPUT_HANDLE), ch[1], Length(ch), numWritten,nil);
      WriteConsoleOutputAttribute(h,@WinAttr, 1, writePos, numWritten);
      Inc(CurrX);
