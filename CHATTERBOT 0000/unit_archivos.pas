...
{ $$    $$ $ $$$ $ $  CAMBIE ESTO $ $ $$  $$  $}
      ...
      // VEMOS SI TIENE MAS DE UNA PALABRA.
      // ASI CUANDO VAYAMOS A COMPARAR CON T1, QUITAMOS EL ESPACIO EN T1.
      for j := 1 to length(palabrasClave) do begin
        if (palabrasClave[j] = ' ') then tieneMasDeUnaPalabra := true;
      end;
        ...
        if (letra <> ' ') then
          ...
          if (Length(palabra) <> 0) then
              begin
                ...
                if (not tieneMasDeUnaPalabra) then t1 := copy(t1, 1, Length(t1)-1);
